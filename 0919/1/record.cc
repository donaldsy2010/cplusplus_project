...
vector< VR > Record::findByPhone(const string &phone,const vector<Record> &vec) {
    vector< VR > temp(5);
    int index = 0;
    for(vector<Record>::const_iterator it = vec.begin(); it != vec.end(); ++it){
        if(phone == (*it).getPhone()){
            temp[index++].push_back(*it);//增加一条记录
        }
    }
    return temp;
}
vector< VR > Record::findByName(const string &name, const vector<Record> &vec) {
    vector< VR> temp(5);
    int index = 0;
    for(vector<Record>::const_iterator it = vec.begin(); it != vec.end(); ++it){
        if(name == (*it).getName()){
...
vector< VR > Record::findByAddr(const string &addr,const vector<Record> &vec) {
    vector< VR > temp(5);
    int index = 0;
    for(vector<Record>::const_iterator it = vec.begin(); it != vec.end(); ++it){
        if(addr == (*it).getAddr()){
